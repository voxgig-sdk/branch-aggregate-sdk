# branch-aggregate-mcp

[MCP](https://modelcontextprotocol.io) server exposing the BranchAggregate SDK as
two agent tools — `branch-aggregate_list` and `branch-aggregate_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/branch-aggregate-mcp)
make build

# 2. Provide credentials via the environment
export BRANCH_AGGREGATE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user branch-aggregate \
  -- /absolute/path/to/branch-aggregate-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./branch-aggregate-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// branch-aggregate_list: first page of records
{ "entity": "analytics" }
{ "entity": "analytics", "query": { } }

// branch-aggregate_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/branch-aggregate-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user branch-aggregate \
     -- "$PWD"/dist/*/branch-aggregate-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `branch-aggregate_list` and `branch-aggregate_load` tools now appear
   in new sessions. Ask the agent to *"list analytics using branch-aggregate"*
   and it calls `branch-aggregate_list` with `{"entity":"analytics"}`.
...
```sh
export BRANCH_AGGREGATE_APIKEY=sk_live_xxx            # API key
export BRANCH_AGGREGATE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `branch-aggregate_list` tool
...
### Call the `branch-aggregate_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `branch-aggregate_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `branch-aggregate_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BRANCH_AGGREGATE_APIKEY` | API key sent with every request. |
| `BRANCH_AGGREGATE_BASE` | Optional override of the API base URL. |
...
```sh
./branch-aggregate-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"branch-aggregate_load","arguments":{"entity":"analytics","query":{"id":1}}}}'
```
