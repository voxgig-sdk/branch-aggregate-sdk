# BranchAggregate Golang SDK



The Golang SDK for the BranchAggregate API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.
...
## Install
```bash
go get github.com/voxgig-sdk/branch-aggregate-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/branch-aggregate-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/branch-aggregate-sdk/go=../branch-aggregate-sdk/go
```
...
import (
    "fmt"
    "os"
    sdk "github.com/voxgig-sdk/branch-aggregate-sdk/go"
)

func main() {
    client := sdk.NewBranchAggregateSDK(map[string]any{
        "apikey": os.Getenv("BRANCH_AGGREGATE_APIKEY"),
    })

    // Load a single analytics — the value is the loaded record.
    analytics, err := client.Analytics(nil).Load(map[string]any{"id": "example_id", "app_id": "example_app_id", "format": "example_format", "limit": 1}, nil)
    if err != nil {
        panic(err)
    }
    ...
    // Create a analytics.
    created, err := client.Analytics(nil).Create(map[string]any{"format": "example_format", "limit": 1, "data_source": "example_data_source", "dimensions": []any{}, "end_date": "example_end_date", "start_date": "example_start_date"}, nil)
    if err != nil {
        panic(err)
    }
    fmt.Println(created)
...
```go
analytics, err := client.Analytics(nil).Load(map[string]any{"id": "example_id", "app_id": "example", "format": "example", "limit": 1}, nil)
if err != nil {
    // handle err
    return
}
...
analytics, err := client.Analytics(nil).Load(
    map[string]any{"id": "test01", "app_id": "example", "format": "example", "limit": 1}, nil,
)
if err != nil {
    panic(err)
}
...
### NewBranchAggregateSDK

```go
func NewBranchAggregateSDK(options map[string]any) *BranchAggregateSDK
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *BranchAggregateSDK
```

Creates a test-mode client with mock transport. Both arguments may be `nil`.

### BranchAggregateSDK methods
...
| `Analytics` | `(data map[string]any) BranchAggregateEntity` | Create an Analytics entity instance. |

### Entity interface (BranchAggregateEntity)

All entities implement the `BranchAggregateEntity` interface.
...
| `enable_install_calculation` | `bool` | If true, then Branch will de-dupe unattributed installs caused by duplicate events from non-opt-in users coming from paid ads (result from [iOS 14.5 privacy changes](https://help.branch.io/faq/docs/ios-14-faqs)). |
| `end_date` | `string` | The end of the interval time range represented as an ISO-8601 complete date. |
| `filters` | `map[string]any` | Keys are same as dimensions. |
...
```go
analytics, err := client.Analytics(nil).Load(map[string]any{"id": "analytics_id", "app_id": "app_id", "format": "format", "limit": 1}, nil)
if err != nil {
    panic(err)
}
...
```
github.com/voxgig-sdk/branch-aggregate-sdk/go/
├── branch-aggregate.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/branch-aggregate-sdk/go`) re-exports everything needed
...
analytics.Load(map[string]any{"id": "example_id", "app_id": "example", "format": "example", "limit": 1}, nil)
