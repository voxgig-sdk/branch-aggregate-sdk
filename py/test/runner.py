# BranchAggregate SDK test runner

from __future__ import annotations
import os
import json

from branchaggregate_sdk.utility.voxgig_struct import voxgig_struct as vs


class BranchAggregateTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BranchAggregateTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BranchAggregateTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BranchAggregateTestRunner.getenv("BRANCH_AGGREGATE_TEST_LIVE")
        override = BranchAggregateTestRunner.getenv("BRANCH_AGGREGATE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BranchAggregateTestRunner.getenv(key)
        ...
        explain = BranchAggregateTestRunner.getenv("BRANCH_AGGREGATE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["BRANCH_AGGREGATE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if BranchAggregateTestRunner._test_control is not None:
            return BranchAggregateTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BranchAggregateTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BranchAggregateTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BranchAggregateTestRunner.load_test_control()
...
def load_env_local():
    BranchAggregateTestRunner.load_env_local()


def env_override(m):
    return BranchAggregateTestRunner.env_override(m)


def entity_data(v):
    return BranchAggregateTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return BranchAggregateTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BranchAggregateTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BranchAggregateTestRunner.load_test_control()


def live_client_options():
    return BranchAggregateTestRunner.live_client_options()


def live_delay_ms():
    return BranchAggregateTestRunner.live_delay_ms()
