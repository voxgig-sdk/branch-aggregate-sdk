...
(0, node_test_1.describe)('AnalyticsDirect', async () => {
    ...
    // `test.live.delayMs`; only sleeps when BRANCH_AGGREGATE_TEST_LIVE=TRUE.
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('BRANCH_AGGREGATE_TEST_LIVE'));
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.BranchAggregateSDK({
...
function liveScenariosActive() { return false && process.env.BRANCH_AGGREGATE_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    const live = 'TRUE' === env.BRANCH_AGGREGATE_TEST_LIVE;
    if (live) {
        const transport = (0, live_runner_1.createLiveTransport)();
        // Merged so the generated fields win: sdk-test-control.json's
        // test.client.options adds to the live client, it does not redirect it.
        const client = new __1.BranchAggregateSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.BRANCH_AGGREGATE_APIKEY,
        }));
        let idmap = env['BRANCH_AGGREGATE_TEST_ANALYTICS_ENTID'];
    ...
    const client = new __1.BranchAggregateSDK({
        base: 'http://localhost:8080',
