...
const SDK_NAME = 'BranchAggregateSDK'
...
function isRunnable(code: string): boolean {
  return (
    /new\s+BranchAggregateSDK\b/.test(code) ||
    /\bBranchAggregateSDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+BranchAggregateSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/BranchAggregateSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
