# BranchAggregate PHP SDK



The PHP SDK for the BranchAggregate API — an entity-oriented client using PHP conventions.
...
```php
<?php
require_once 'branchaggregate_sdk.php';

$client = new BranchAggregateSDK([
    "apikey" => getenv("BRANCH_AGGREGATE_APIKEY"),
]);
```
...
```php
require_once 'branchaggregate_sdk.php';
$client = new BranchAggregateSDK($options);
```
...
| `filters` | `array` | Keys are same as dimensions. |
...
```
php/
├── branchaggregate_sdk.php          -- Main SDK class
...
The main class (`branchaggregate_sdk.php`) exports the SDK class
