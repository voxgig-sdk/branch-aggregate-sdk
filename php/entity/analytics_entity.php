...
// BranchAggregate SDK Analytics entity
...
class AnalyticsEntity
{
    ...
    {
        if ($args) {
            $this->_data = BranchAggregateHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = BranchAggregateHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   SDK boundary; throws BranchAggregateError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = BranchAggregateHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
