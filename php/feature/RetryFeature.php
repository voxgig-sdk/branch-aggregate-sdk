...
// BranchAggregate SDK retry feature
...
class BranchAggregateRetryFeature extends BranchAggregateBaseFeature
{
    ...
    public function init(BranchAggregateContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (BranchAggregateContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(BranchAggregateContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(BranchAggregateContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
