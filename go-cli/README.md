# branch-aggregate-cli

boru-driven command-line client **and** interactive REPL for the BranchAggregate
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/branch-aggregate-cli)
make build

# 2. See usage (words, entities, env vars)
./branch-aggregate-cli --help

# 3. Provide credentials once, via the environment
export BRANCH_AGGREGATE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./branch-aggregate-cli load 1 analytics            # {id:1} shorthand
./branch-aggregate-cli load '{id:1}' analytics       # explicit match map

# 5. Override the API base URL for a single call
BRANCH_AGGREGATE_BASE=https://api.example.com ./branch-aggregate-cli load 1 analytics

# 6. No arguments -> interactive REPL
./branch-aggregate-cli
branch-aggregate> load 1 analytics
branch-aggregate> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/branch-aggregate-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/branch-aggregate-cli load 1 analytics
   ```
...
```sh
./branch-aggregate-cli load 1 analytics          # scalar shorthand for {id:1}
./branch-aggregate-cli load '{id:1}' analytics     # explicit match map
```
...
./branch-aggregate-cli load 1 analytics
```
...
Run with no arguments to open a REPL (prompt `branch-aggregate>`). Each line is
evaluated as its own boru expression:

```text
$ ./branch-aggregate-cli
branch-aggregate> load 1 analytics
branch-aggregate> /help
branch-aggregate> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/branch-aggregate-cli`. |
...
(`./branch-aggregate-cli <expr>`) and interactively (the REPL), and expressions compose the
