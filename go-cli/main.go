// Command BranchAggregate-cli is a CLI + REPL for the BranchAggregate SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      BRANCH_AGGREGATE_APIKEY (api key), BRANCH_AGGREGATE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: BRANCH_AGGREGATE_APIKEY carries the API key and
	// BRANCH_AGGREGATE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("BRANCH_AGGREGATE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("BRANCH_AGGREGATE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewBranchAggregateSDK(opts)
