# BranchAggregate JavaScript SDK



The JavaScript SDK for the BranchAggregate API — an entity-oriented client with full async/await support.
...
```js
const { BranchAggregateSDK } = require('@voxgig-sdk/branch-aggregate-js')
...
```js
const analytics = await client.Analytics().load({ id: 'analytics_id', app_id: 'example_app_id', format: 'example_format', limit: 1 })
...
| `filters` | `Object` | Keys are same as dimensions. |
...
branch-aggregate/
├── src/
│   ├── BranchAggregateSDK.js        # Main SDK class
