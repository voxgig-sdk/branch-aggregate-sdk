...
// BranchAggregate SDK feature test
//
...
class FtCtrl extends BranchAggregateControl
{
...
class FtClient
{
    ...
    private BranchAggregateUtility $utility;

    public function __construct(BranchAggregateUtility $utility, array $options)
    {
    ...
    public function get_utility(): BranchAggregateUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BranchAggregateContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new BranchAggregateUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (BranchAggregateContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BranchAggregateConfig::make_feature($name);
    ...
    private static function buildUrl(BranchAggregateSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BranchAggregateError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BranchAggregateResponse($fetched);
        }
    ...
    private function fail(BranchAggregateContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BranchAggregateContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
