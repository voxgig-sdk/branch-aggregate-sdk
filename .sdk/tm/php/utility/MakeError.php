...
class BranchAggregateMakeError
{
    public static function call(?BranchAggregateContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new BranchAggregateContext([], null);
        }
        $op = $ctx->op ?? new BranchAggregateOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new BranchAggregateResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof BranchAggregateError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new BranchAggregateError('', $msg, $ctx);
        ...
        if ($err instanceof BranchAggregateError) {
