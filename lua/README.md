# BranchAggregate Lua SDK



The Lua SDK for the BranchAggregate API — an entity-oriented client using Lua conventions.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/branch-aggregate-sdk/releases)),
...
```lua
local analytics, err = client:Analytics():load({ id = "example_id", app_id = "example_app_id", format = "example_format", limit = 1 })
...
```lua
-- Create
local created, err = client:Analytics():create({ format = "example_format", limit = 1, data_source = "example_data_source", dimensions = {}, end_date = "example_end_date", start_date = "example_start_date" })
if err then error(err) end
...
```lua
local analytics, err = client:Analytics():load({ id = "example_id", app_id = "example", format = "example", limit = 1 })
if err then error(err) end
```
...
local result, err = client:Analytics():load({ id = "test01", app_id = "example", format = "example", limit = 1 })
...
| `filters` | `table` | Keys are same as dimensions. |
...
```lua
local analytics, err = client:Analytics():load({ id = "analytics_id", app_id = "app_id", format = "format", limit = 1 })
```
...
```
lua/
├── branch-aggregate_sdk.lua    -- Main SDK module
...
The main module (`branch-aggregate_sdk`) exports the SDK constructor
