...
require("branch-aggregate_types")
...
local BranchAggregateSDK = {}
BranchAggregateSDK.__index = BranchAggregateSDK
...
BranchAggregateSDK._make_feature = _make_feature


function BranchAggregateSDK.new(options)
  local self = setmetatable({}, BranchAggregateSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function BranchAggregateSDK:options_map()
...
function BranchAggregateSDK:get_utility()
  return Utility.copy(self._utility)
end


function BranchAggregateSDK:get_root_ctx()
  return self._rootctx
end


function BranchAggregateSDK:prepare(fetchargs)
...
function BranchAggregateSDK:direct(fetchargs)
...
function BranchAggregateSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function BranchAggregateSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "BranchAggregateSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function BranchAggregateSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function BranchAggregateSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "BranchAggregateSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function BranchAggregateSDK:Analytics(data)
...
function BranchAggregateSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = BranchAggregateSDK.new(sdkopts)
  sdk.mode = "test"
